...
class TeamService:
    ...
    def create_team(cls, new_team_data):
        ...
        try:
            ...
            cls.logger().info(f"User #{user.user_id} assigned as team #{new_client.client_id} #{new_user_by_team.role}.")
        except exc.IntegrityError:
            db.session.rollback()
            if db.session.query(TeamTableEntry).filter(TeamTableEntry.team_name == new_team_data.team_name()).first():
                cls.logger().info(f"Failing to create team {new_team_data.team_name()}. Name already in use for other team.")
