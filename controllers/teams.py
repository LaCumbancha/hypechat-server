...
def add_forbidden_word():
    ...
    req = ClientRequest(request)
    response = TeamService.add_forbidden_word(req.add_forbidden_word())
...
def forbidden_words(team_id):
    ...
    req = ClientRequest(request)
    words = TeamService.forbidden_words(req.team_authentication(team_id))
...
def delete_forbidden_word(team_id, word_id):
    ...
    req = ClientRequest(request)
    response = TeamService.delete_forbidden_word(req.delete_forbidden_word(team_id, word_id))
    return jsonify(response.json()), response.status_code()
